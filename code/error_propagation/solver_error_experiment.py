...
def initial_params():
    return {
        "nn_params": init_network_params([2, 64, 64, 2], random.key(0)),
...
def train_model(method, ratio, data, num_epochs, step_size, chunk_size):
    h = obs_dt / ratio
...
def evaluate_model(trained, method, predict_ratio, data, experiment_type):
    ...
    pred_train = rollout_batch(y0_batch, h, model_rhs, params, steps, method)[
        :, ::predict_ratio, :
    ]
    pred_val = rollout_batch(y0_validation, h, model_rhs, params, steps, method)[
        :, ::predict_ratio, :
    ]
    pred_extrap = rollout_batch(y0_batch, h, model_rhs, params, extrap_steps, method)[
        :, ::predict_ratio, :
    ]
...
def run_prediction_sweep(output_dir="prediction_sweep_results"):
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    for train_method in solvers:
        for train_ratio in [1, 2, 4, 8, 16]:
            trained = train_model(
                train_method,
                train_ratio,
                data,
            ...
            all_trained[(train_method.__name__, train_ratio)] = trained

    rows = []
    details = {}
    predict_ratios = [1, 2, 4, 8, 16]

    for (train_method_name, train_ratio), trained in all_trained.items():
        for pred_method in solvers:
            for predict_ratio in predict_ratios:
                row, curves = evaluate_model(
                    trained=trained,
                    method=pred_method,
                    predict_ratio=predict_ratio,
                    data=data,
                    experiment_type="prediction_sweep",
                )
                rows.append(row)
                key = (
                    train_method_name,
                ...
                details[key] = curves
                print(
                    "train =", train_method_name,
                    "train_ratio =", train_ratio,
                    "pred =", pred_method.__name__,
                    "predict_ratio =", predict_ratio,
                    "h_predict =", row["h_predict"],
                    "train_mse =", row["train_mse"],
                    "val_mse =", row["validation_mse"],
                    "extrap_mse =", row["extrapolate_mse"],
                    "instability =", row["instability_rate"],
                )
